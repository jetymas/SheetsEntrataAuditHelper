const { getCookie, setCookie } = require('../src/js/cookieHelper');

describe('cookieHelper', () => {
  beforeEach(() => {
    global.chrome = { cookies: { get: jest.fn(), set: jest.fn() }, runtime: { lastError: null } };
  });

  test('getCookie resolves cookie value', async () => {
    const fakeCookie = { value: 'abc' };
    chrome.cookies.get.mockImplementation(({ url, name }, cb) => cb(fakeCookie));
    const val = await getCookie('http://example.com', 'session');
    expect(val).toBe('abc');
  });

  test('getCookie resolves null if cookie not found', async () => {
    chrome.cookies.get.mockImplementation(({ url, name }, cb) => cb(null));
  ...
  test('setCookie sets cookie and resolves', async () => {
    ...
    const result = await setCookie({ url: 'http://example.com', name: 'n', value: 'v' });
  ...
  test('setCookie rejects when chrome.runtime.lastError', async () => {
    ...
    await expect(setCookie({ url: 'u', name: 'n', value: 'v' })).rejects.toBe(err);
  });

  test('setCookie includes expirationDate when provided', async () => {
    chrome.cookies.set.mockImplementation((details, cb) => cb(details));
    const result = await setCookie({ url: 'http://example.com', name: 'n', value: 'v', expirationDate: 999 });
