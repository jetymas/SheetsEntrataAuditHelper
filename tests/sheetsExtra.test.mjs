...
describe("Sheets Helper Functions", () => {
  ...
  test("updateSheetCell throws on non-OK status", async () => {
    ...
    await expect(updateSheetCell("sid", "Sheet", "A1", "val")).rejects.toThrow(
  ...
  test("addSheetComment throws if sheet not found", async () => {
    fetchMock.mockResponseOnce(JSON.stringify({ sheets: [] }), {
      status: 200,
    });
    await expect(addSheetComment("sid", "SheetX", "A1", "c")).rejects.toThrow(
      "Sheet SheetX not found",
    );
  });
  ...
  test("addSheetComment throws on metadata fetch error", async () => {
    ...
    await expect(
      addSheetComment("sid", "Sheet", "A1", "comment"),
    ).rejects.toThrow("Failed to get sheet metadata: Not Found");
  });
  ...
  test("addSheetComment throws on batch update error", async () => {
    ...
    await expect(
      addSheetComment("sid", "Sheet", "A1", "comment"),
    ).rejects.toThrow("Failed to add comment: BatchFail");
  });
});
