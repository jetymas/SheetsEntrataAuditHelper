...
let server, browser, extensionId, popupPage;
let _page;
...
describe("Column AO User Confirmation E2E", () => {
  beforeAll(async () => {
    ...
    popupPage.on("console", (msg) => {
      for (const arg of msg.args()) {
        console.log(`POPUP LOG [${msg.type()}]: ${arg}`);
      }
    });
  ...
  it("prompts user for AO column and processes confirmation", async () => {
    try {
      ...
      await popupPage.waitForSelector("#ea-popup-verification-dialog", {
        timeout: 10000,
      });
      const dialogText = await popupPage.$eval(
        "#ea-popup-verification-dialog",
        (el) => el.textContent,
      );
      ...
      const logs = await popupPage.evaluate(() =>
        Array.from(window.console.logs || []),
      );
