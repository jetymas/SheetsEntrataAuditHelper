...
import {
  ...
  createVerificationDialog
    ...
    console.warn("Manual scroll to text failed");
    return false;
  }
}
...
function navigateToDocumentsTab() {
  ...
  const navItems = Array.from(
    document.querySelectorAll(
      "nav a, .navbar a, .tabs a, .tab, .nav-item, .menu-item, [role=\"tab\"]",
    ),
  );
...
function findAndClickLeaseDocument() {
  ...
  console.log(
    "Found " +
      leaseRows.length +
      " potential lease documents with pattern \"* Lease - Signed\"",
  );
  ...
  console.log(
    "No documents matching \"* Lease - Signed\" pattern found, trying generic lease search...",
  );
...
function getCurrentPageType() {
  ...
  if (url.includes("customers_system") || url.includes("module=customers")) {
    ...
    const hasResidentList =
      document.querySelector(
        ".resident-list, .residents-list, [data-testid*=\"resident\"]",
      ) ||
      document.querySelector("table.residents, table.list") ||
      Array.from(document.querySelectorAll("h1, h2, .page-title")).some(
        (el) =>
  ...
  if (url.includes("/resident/")) {
    ...
    const hasDocumentsTab = Array.from(
      document.querySelectorAll("a, div, button, li, span, nav a"),
    ).some(
      (el) =>
        el.textContent &&
        (el.textContent.trim() === "Documents" ||
    ...
    const hasDocumentsList =
      document.querySelector(
        ".documents-list, .document-table, [data-testid=\"documents-list\"]",
      ) ||
      Array.from(document.querySelectorAll("tr, td, div")).some(
        (el) =>
          el.textContent &&
          el.textContent.includes("Lease") &&
          (el.textContent.includes("Signed") ||
            el.textContent.includes("e-Sign")),
      );
...
async function setupAudit(auditType) {
  ...
  try {
    ...
    const isResidentsPage =
      currentUrl.includes("customers_system") ||
    ...
    if (!isResidentsPage) {
      ...
      const potentialResidentElements = document.querySelectorAll(
        "table, tbody, tr, td, div.list",
      );
      console.log(
        `Found ${potentialResidentElements.length} potential resident elements`,
      );

      // Log what we found to help debug
      Array.from(potentialResidentElements)
        .slice(0, 5)
        .forEach((el, idx) => {
          console.log(
            `Element ${idx}:`,
    ...
    if (!viewSetupSuccess) {
      console.warn(
        "Could not set up optimal residents view, but continuing...",
      );
    }

    // Document what we found for debugging purposes
    const residentRows = document.querySelectorAll(
      "tr, .resident-row, .list-item",
    );
    console.log(`Found ${residentRows.length} potential resident rows`);

    // Document a few sample resident names if available
    const sampleNames = Array.from(residentRows)
      .slice(0, 3)
      .map((row) => {
...
async function goToFirstPage() {
  ...
  const firstPageButtons = Array.from(
    document.querySelectorAll("a, button, span, div"),
  ).filter((el) => {
    if (!el.textContent) return false;
...
async function setupResidentsView() {
  ...
  const nameHeaders = Array.from(
    document.querySelectorAll("th, td, div.header"),
  ).filter(
    (el) =>
      el.textContent &&
  ...
  if (nameHeaders.length > 0) {
    ...
    console.log(
      "Found name column header, clicking to sort:",
      nameHeaders[0].textContent,
    );
    nameHeaders[0].click();
    await wait(1000);
  ...
  const sortOptions = Array.from(
    document.querySelectorAll("select, button, a"),
  ).filter(
    (el) =>
      el.textContent &&
  ...
  if (sortOptions.length > 0) {
    ...
    const nameOptions = Array.from(
      document.querySelectorAll("option, li, a, div"),
    ).filter(
      (el) =>
        el.textContent &&
        (el.textContent.toLowerCase().includes("name") ||
...
async function processResident(resident, record) {
  ...
  if (pageType !== "residentsList") {
    console.warn(
      "[LeaseAudit] Not on Residents list. Current pageType =",
      pageType,
    );
  ...
  const residentFound = await findAndClickResident(
    resident.firstName,
    resident.lastName,
    true,
  );
  if (!residentFound) {
    return {
      success: false,
      error: "Resident not found in list (checked all pages)",
    };
  }
  ...
  const docsLoaded = await poll(
    () => getCurrentPageType() === "documentsTab",
    10000,
  );
  if (!docsLoaded) {
  ...
  const pdfLoaded = await poll(
    () => getCurrentPageType() === "pdfViewer",
    10000,
  );
  if (!pdfLoaded) {
  ...
  console.log(
    "✔ PDF text extracted:",
    contentState.pdfText.substring(0, 100) + "...",
  );
...
const messageHandlers = {
  async verifyField(message, sender, sendResponse) {
    // Patch verifyField to use dynamic modules and handle user confirmation UI
    // The original logic from verifyField function
    const { column, module, expectedValue, sheetColumn } = message;
    await verifyField(column, module, expectedValue, sheetColumn);
    sendResponse({ status: "done" });
    return true;
  },
  // Add other actions as needed, e.g. ping, etc.
...
// Message handling is now modularized using registerContentMessaging from content-messaging.js
    sendResponse({
      received: true,
    ...
    console.warn("Could not send initial response acknowledgment");
  }
  ...
  const keepAliveInterval = setInterval(() => {
    ...
    try {
      ...
      console.warn("Keep-alive response failed, clearing interval");
  ...
  if (message.action === "setupAudit") {
    ...
    // TODO: Implement processResident logic here
    clearInterval(keepAliveInterval);
    return true; // Will respond asynchronously
  } else if (message.action === "processResident") {
    console.log("Processing resident:", message.resident);
    ...
    processResident(message.resident, message.record)
      ...
      .catch((error) => {
        ...
        sendResponse({
          success: false,
          error: error.message || "Unknown error processing resident",
        });
      });
    return true; // Will respond asynchronously
    ...
    verifyField(
      ...
      .catch((error) => {
        ...
        sendResponse({
          status: "error",
          error: error.message || "Unknown error during field verification",
        });
      });
    return true; // Will respond asynchronously
...
// === Column Modules Loader (Static Import) ===
