...
function processQueuedMessages() {
  ...
  while (messageQueue.length > 0) {
    ...
    try {
      ...
      sendResponse({ status: 'error', error: 'Failed to process queued message' });
    }
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (!backgroundLoaded) {
    console.log(`Background.js not loaded yet, queuing message: ${message.action}`);
    
    // For certain critical messages, provide an immediate response
    if (message.action === 'getAuditState' || message.action === 'getStatus') {
      sendResponse({ 
        status: 'loading',
        message: 'Background script is still initializing, please try again in a moment'
      });
      return true;
    }
