/**
 * Column G Module - Floor Plan
 * Verifies the floor plan in Entrata (DN) against the lease document
 */
...
const ColumnG = {
  ...
  async run(row, col, context) {
    try {
      ...
      // Use regex to extract the value after "Floor Plan:"
      ...
      const normalizedPdfValue = ColumnHelpers.normalizeValue(pdfValue);
      const normalizedExpectedValue = ColumnHelpers.normalizeValue(expectedValue);
      ...
      return {
        ...
        // This is informational only - no checkbox to mark
