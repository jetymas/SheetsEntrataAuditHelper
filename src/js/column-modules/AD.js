/**
 * Column AD Module - Lease Signed by Resident
 * Verifies that the lease is signed by the resident
 */
...
const ColumnAD = {
  ...
  pdfSelector: 'Resident Signature',
  ...
  async run(row, col, context) {
    try {
      ...
      // Look for resident signature indicators
  ...
  displayData(row, col, context) {
    if (!context.result || !context.result.success) {
      return {
        fieldName: this.name,
        message: 'Please confirm if the lease is signed by the resident.',
        requiresUserConfirmation: true
      };
    }
    ...
    return {
      ...
      message: context.result.signatureFound 
        ? 'Please confirm that you can see the resident signature on this page.'
        : 'Please verify if the lease is signed by the resident. No signature indicator was found automatically.',
      requiresUserConfirmation: true
    };
  }
};
