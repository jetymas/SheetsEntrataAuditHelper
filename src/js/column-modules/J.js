/**
 * Column J Module - Lease Start Date in DN
 * Verifies the lease start date in Entrata (DN) against the lease document
 */
...
const ColumnJ = {
   ...
   */
  id: 'leaseStartDate',
  name: 'Lease Start Date in DN',
  pdfSelector: 'Start Date',
  sheetColumn: 'Lease Start Date in DN',
  resultColumn: 'J',
  resultCheckboxColumn: 'K', // Linked to column K for checkbox
  ...
  async run(row, col, context) {
    try {
      ...
      // Find the lease start date in the PDF
      ...
      // Use regex to extract the value after "Start Date:"
