/**
 * Column N Module - Base Rate in DN
 * Verifies the base rent rate in Entrata (DN) against the lease document
 */
...
const ColumnN = {
   ...
   */
  id: 'baseRate',
  name: 'Base Rate in DN',
  pdfSelector: 'Rent',
  sheetColumn: 'Base Rate in DN',
  resultColumn: 'N',
  resultCheckboxColumn: 'O', // Linked to column O for checkbox
  ...
  async run(row, col, context) {
    try {
      ...
      // Find the value in the PDF by searching for various rent-related terms
      const searchTerms = ['Monthly Installment', 'Rent', 'Base Rent'];
      let pdfValue = null;
      ...
      if (!pdfValue) {
        ...
        for (let i = 0; i < lines.length; i++) {
          if (searchTerms.some(term => lines[i].includes(term))) {
            // Found a line with a rent-related term, look for currency values in this line or next few lines
