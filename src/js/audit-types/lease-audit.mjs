 ...
 */
import BaseAuditType from './base-audit.mjs/index.js';
...
class LeaseAudit extends BaseAuditType {
  ...
  async setUp() {
    try {
      ...
      console.log('Current URL:', window.location?.href || 'Not accessible from background script');
      ...
      try {
        await new Promise((resolve, reject) => {
          chrome.tabs.get(activeTab.id, (tab) => {
            console.log('Tab get result:', tab ? `Tab exists with URL: ${tab.url}` : 'Tab not found');
            
            if (chrome.runtime.lastError) {
              console.error('Tab check chrome.runtime.lastError:', chrome.runtime.lastError);
        ...
        throw new Error('Entrata tab is no longer accessible. Please try again.');
      }
      
      console.log('Tab is valid, sending setupAudit message to content script...');
      ...
      try {
        await new Promise((resolve, reject) => {
          const pingTimeoutId = setTimeout(() => {
            reject(new Error('Content script ping timed out - script may not have loaded'));
          }, 5000);
          ...
          chrome.tabs.sendMessage(activeTab.id, { action: 'ping' }, (response) => {
            ...
            if (chrome.runtime.lastError) {
              console.error('Ping chrome.runtime.lastError:', chrome.runtime.lastError);
              reject(new Error('Ping error: ' + chrome.runtime.lastError.message));
            } else if (!response) {
        ...
        throw new Error(`Content script not loaded. This usually means the extension's content scripts didn't load on the Entrata page. Please try refreshing the page or reinstalling the extension.`);
      }
      ...
      const setupResult = await new Promise((resolve, reject) => {
        ...
        const timeoutId = setTimeout(() => {
          reject(new Error('Timeout waiting for content script response - the page may not have loaded correctly'));
        }, 15000);
        ...
        try {
          chrome.tabs.sendMessage(activeTab.id, {
            ...
            if (chrome.runtime.lastError) {
              console.error('setupAudit chrome.runtime.lastError:', chrome.runtime.lastError);
              ...
              reject(new Error('No response from content script - the page may not have loaded correctly'));
            } else {
              console.log('Received setupAudit response:', JSON.stringify(response));
              resolve(response);
            }
          });
        } catch (error) {
      ...
      if (!setupResult || !setupResult.success) {
        ...
        throw new Error(setupResult?.error || 'Failed to set up Entrata environment');
      }
  ...
  async findNext() {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        chrome.tabs.update(entrataTabId, { active: true }, (tab) => {
          if (chrome.runtime.lastError) {
      ...
      console.log('Tab activated, sending processResident message to tab', entrataTabId);
      ...
      const response = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout - no response from content script')), 30000))
      ]);
      
      if (!response || !response.success) {
        // If resident not found, log detailed error
        console.error('Failed to process resident:', response?.error || 'Unknown error');
