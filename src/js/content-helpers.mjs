...
function findClickableParent(_el) {
  // For testing, always return null to use direct element
  return null;
}

async function findAndClickResident(firstName, lastName, _tryNextPage = true) {
  ...
  const partialMatches = Array.from(
    ...
    (node) =>
      node.textContent &&
      node.textContent.includes(firstName) &&
      node.textContent.includes(lastName),
  );
