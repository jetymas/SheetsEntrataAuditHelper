...
helperFiles.forEach(file => {
  ...
  let ast;
  try { ast = recast.parse(code, { parser }); }
    ...
    visitVariableDeclaration(pathNode) {
      ...
      if (decl.init && decl.init.type === 'CallExpression' && decl.init.callee.name === 'require') {
        ...
        if (id.type === 'Identifier') {
          ...
          importDecl = b.importDeclaration(
            id.properties.map(p => b.importSpecifier(b.identifier(p.key.name))),
            b.literal(source)
          );
        }
        if (importDecl) { pathNode.replace(importDecl); return false; }
      }
  ...
  recast.types.visit(ast, {
    visitExpressionStatement(pathNode) {
      ...
      if (expr.type === 'AssignmentExpression' && expr.left.type === 'MemberExpression' && expr.left.object && expr.left.object.name === 'module') {
        pathNode.prune(); return false;
      }
      this.traverse(pathNode);
    }
  });
  ...
  const exports = ['injectChromeIdentityStub','launchExtensionWithServer','setupRequestInterception','captureScreenshotsOnFailure','autoStubAllPages'];
  const exportDecl = b.exportNamedDeclaration(null, exports.map(name => b.exportSpecifier(b.identifier(name), b.identifier(name))));
